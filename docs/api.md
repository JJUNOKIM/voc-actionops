...
`feedbackCount`는 해당 날짜에 새로 접수된 건수가 아니라 집계 시점에 이슈에 연결된 누적 건수다. 기록이 없는 날짜는 응답에 포함하지 않으며, 화면에서도 0건으로 보간하지 않는다. `analyzedFeedbackCount`가 0인 경우 부정 비율은 화면에 `미분석`으로 표시한다.

`resolvedDate`는 집계 기준 시간대인 `Asia/Seoul`로 변환한 해결일이며, 미해결 이슈는 `null`이다. 기존 `resolvedAt`은 서버의 로컬 시각으로 기록되므로 서버 기본 시간대로 해석한 뒤 변환한다. 데이터가 있는 서버의 시간대를 변경할 때는 기존 시각 데이터의 변환도 함께 검토해야 한다.

이슈 상세의 해결 전후 비교는 조회 기간 안에서 `resolvedDate` 이전의 마지막 기록과 이후의 최신 기록을 사용한다. snapshot에는 집계 시각이 없어 해결 당일 기록은 비교에서 제외한다. 양쪽 기록이 모두 있어야 비교하며, 누적값의 변화 자체를 신규 접수량이나 조치의 인과적 효과로 해석하지 않는다.
